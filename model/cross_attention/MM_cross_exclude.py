...
# 데이터셋 클래스
class MultiStockDatasetExcludeWindow(Dataset):
    def __init__(self, csv_path, img_base_path, transform=None, exclude_window=None, label_col='Signal_origin', mode='train', train_end_idx=None):
        self.csv_data = pd.read_csv(csv_path)
        self.img_base_path = img_base_path
        self.transform = transform
        self.exclude_window = str(exclude_window)
        self.label_col = label_col
        self.mode = mode
        
        # 사용 가능한 윈도우 목록 (exclude_window 제외)
        all_windows = [5, 20, 60, 120]
        self.window_sizes = [w for w in all_windows if w != int(self.exclude_window)]
...
# 모델 클래스
class StockPredictorExcludeWindow(nn.Module):
    def __init__(self, exclude_window, input_size=25, hidden_unit=256, num_layers=4, num_attention_heads=16, intermediate_size=512, dropout_prob=0.5, mhal_num_heads=16, mlp_hidden_unit=512):
        super(StockPredictorExcludeWindow, self).__init__()
        ...
        # 사용 가능한 윈도우 목록 (exclude_window 제외)
        all_windows = ['5', '20', '60', '120']
        self.windows = [w for w in all_windows if w != str(exclude_window)]
        
        # LSTM 모듈 (제외된 윈도우 제외)
        self.lstm_dict = nn.ModuleDict({
            window: nn.LSTM(input_size=self.input_size, 
    ...
    def get_model_name(self):
        model_name = f"MM_Causal_ViT_LSTM_ExcludeWindow{self.exclude_window}_"
        ...
        model_name += f"(MLP_{self.hidden_unit*len(self.windows)}_{self.mlp_hidden_unit})"
        return model_name
...
if __name__ == "__main__":
    ...
    for exclude_window in exclude_windows:
        for ticker in tickers:
            csv_path = f'./data/csv/TA_csv/{ticker}.csv'
            img_base_path = f'./data/candle_img/{ticker}'
            
            for label_col in label_cols:
                print(f'Processing {ticker} for {label_col} excluding window {exclude_window}...')
                train_dataset = MultiStockDatasetExcludeWindow(
                    csv_path, img_base_path, transform=transform, exclude_window=exclude_window, 
                    label_col=label_col, mode='train', train_end_idx=train_end_idx
                )
                test_dataset = MultiStockDatasetExcludeWindow(
                    csv_path, img_base_path, transform=transform, exclude_window=exclude_window, 
                ...
                model = StockPredictorExcludeWindow(
                    exclude_window=exclude_window, input_size=len(train_dataset.ta_cols)
                ).to(device)
                ...
                loaded_model = StockPredictorExcludeWindow(
                    exclude_window=exclude_window, input_size=len(train_dataset.ta_cols)
                ).to(device)
