...
class StockPredictorLSTM(nn.Module):
    def __init__(self, input_size=25, hidden_unit=256, num_layers=4, dropout_prob=0.5, mhal_num_heads=16, mlp_hidden_unit=512):
    ...
    def forward(self, ta_dict):
        # TA 모달리티: 모든 윈도우의 LSTM 출력 융합
        ta_outputs = []
        for window in ['5', '20', '60', '120']:
...
def train_model(model, train_loader, criterion, optimizer, num_epochs, device):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        correct = 0
        total = 0
        for ta_dict, _, labels in train_loader:  # img_dict 무시
            ta_dict = {k: v.to(device) for k, v in ta_dict.items()}
            labels = labels.to(device)

            optimizer.zero_grad()
            outputs = model(ta_dict).squeeze(1)
...
def test_model(model, test_loader, criterion, device):
    ...
    with torch.no_grad():
        for ta_dict, _, labels in test_loader:  # img_dict 무시
            ta_dict = {k: v.to(device) for k, v in ta_dict.items()}
            labels = labels.to(device)
...
if __name__ == "__main__":
    ...
    for ticker in tickers:
        ...
        for label_col in label_cols:
            ...
            model = StockPredictorLSTM(input_size=len(train_dataset.ta_cols)).to(device)  # 수정
            ...
            loaded_model = StockPredictorLSTM(input_size=len(train_dataset.ta_cols)).to(device)  # 수정
