...
# 모델 클래스
class StockPredictorViT(nn.Module):
    def __init__(self, hidden_unit=256, num_layers=4, num_attention_heads=16, intermediate_size=512, dropout_prob=0.5, mhal_num_heads=16, mlp_hidden_unit=512):
        super(StockPredictorViT, self).__init__()
        ...
        # 윈도우별 출력을 융합하기 위한 Attention 레이어
...
def train_model(model, train_loader, criterion, optimizer, num_epochs, device):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        correct = 0
        total = 0
        for _, img_dict, labels in train_loader:  # ta_dict 무시
            img_dict = {k: v.to(device) for k, v in img_dict.items()}
            labels = labels.to(device)

            optimizer.zero_grad()
            outputs = model(img_dict).squeeze(1)
...
def test_model(model, test_loader, criterion, device):
    ...
    with torch.no_grad():
        for _, img_dict, labels in test_loader:  # ta_dict 무시
            img_dict = {k: v.to(device) for k, v in img_dict.items()}
            labels = labels.to(device)
...
if __name__ == "__main__":
    ...
    for ticker in tickers:
        ...
        for label_col in label_cols:
            ...
            model = StockPredictorViT().to(device)  # 수정
            ...
            loaded_model = StockPredictorViT().to(device)  # 수정
